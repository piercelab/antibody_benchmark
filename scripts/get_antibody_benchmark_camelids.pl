...
foreach my $line (@list_lines)
{
    ...
    if (($hchain eq "NA") || ($lchain ne "NA") || ($ag_chain eq "NA")) { next; }
	...
	open(TMP, "> temp2.fa") || die("unable to open temp fa file\n");
	print TMP "> $pdb $chn\n";
	print TMP "$seq\n";

	# run BLAST
	my $blast_cmd = $blast_exe . " -query temp2.fa -db $seqres_file";
	my $blout = `$blast_cmd`;
	# print $blout;
...
#    print $chain_matches[0] . "\n" . $chain_matches[1] . "\n";
    ...
    my $ub_ag_matches = "";
    my %ag_checked = ();
    my @tmp_matches = split("\t", $chain_matches[1]);
